...
-- Para un estudiante determinado que, ha cerrado en alguna carrera, dar el nombre de los estudiantes que llevaron con el todos los cursos.
/* 
Carne Prueba
201710130
201804123
...
SELECT DISTINCT
    ...
    e2.NOMBRE
FROM ESTUDIANTE e1
INNER JOIN ASIGNACION a1
    ON a1.ESTUDIANTE_ID_ESTUDIANTE = e1.ID_ESTUDIANTE
INNER JOIN ASIGNACION a2
    -- EL <> es diferente de es != en SQL
    ON a2.ESTUDIANTE_ID_ESTUDIANTE <> a1.ESTUDIANTE_ID_ESTUDIANTE
   AND a2.PENSUM_CURSO_ID_CURSO = a1.PENSUM_CURSO_ID_CURSO
INNER JOIN ESTUDIANTE e2
    ON e2.ID_ESTUDIANTE = a2.ESTUDIANTE_ID_ESTUDIANTE
WHERE e1.ID_ESTUDIANTE = 201804123
  AND EXISTS (
        SELECT 1
        FROM INSCRIPCION i
        WHERE i.ESTUDIANTE_ID_ESTUDIANTE = e1.ID_ESTUDIANTE
          AND i.ESTADO = 'CERRADA'
  )
ORDER BY e2.NOMBRE;
