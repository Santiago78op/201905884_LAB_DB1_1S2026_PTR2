# Manual de Procedimiento de la carga de datos a la DB - Versión Revisada

## Indice
- [Manual de Procedimiento de la carga de datos a la DB - Versión Revisada](#manual-de-procedimiento-de-la-carga-de-datos-a-la-db---versión-revisada)
  - [Indice](#indice)
  - [Resumen de Errores Encontrados](#resumen-de-errores-encontrados)
...
A continuación se listan los errores detectados en el `manual.md` original, verificados contra el archivo `DATA_inicial.xlsx`:

| # | Tabla / Sección | Error | Corrección |
|---|---|---|---|
| 1 | Índice / Sección de comandos | Typo: se escribe "SQL **DLL** Commands" | Debe decir "SQL **DDL** Commands" |
| 2 | `CORRELATIVO` | El SQL usa el campo `NUMERO_CORRELATIVO`, pero la descripción y los datos del Excel usan `NO_EXAMEN` | Renombrar el campo en el SQL a `NO_EXAMEN` |
| 3 | `RESPUESTA_USUARIO` | La FK `FK_RESP_USUARIO_PREGUNTA` referencia a `PREGUNTAS_PRACTICO(ID_PREGUNTA_PRACTICO)` siendo que esta tabla almacena respuestas del examen **teórico** | Debe referenciar a `PREGUNTAS(ID_PREGUNTA)` |
| 4 | `RESPUESTA_USUARIO` | El `CONSTRAINT UQ_RESP_USUARIO_EXAMEN_PREG` está fuera del bloque `CREATE TABLE` con sintaxis inválida (`ADD CONSTRAINT` suelto sin `ALTER TABLE`) | Mover el constraint dentro del `CREATE TABLE` |
| 5 | `RESPUESTA_USUARIO` | Las FK no incluyen `ON DELETE CASCADE` pese a que la descripción del manual lo indica | Agregar `ON DELETE CASCADE` a ambas FK |
| 6 | `RESPUESTA_PRACTICO_USUARIO` | Las FK no incluyen `ON DELETE CASCADE` pese a que la descripción del manual lo indica | Agregar `ON DELETE CASCADE` a ambas FK |
