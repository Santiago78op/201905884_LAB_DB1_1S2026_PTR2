...
- **TIPO_TRAMITE:** Es un campo de texto que almacena el tipo de trámite del examen (por ejemplo, "Licencia de Conducir", "Renovación de Licencia", etc.).
...
- **GENERO:** Es un campo de texto que almacena el género del usuario que realizó el trámite del examen (por ejemplo, "M" para masculino, "F" para femenino, etc.).
...
Como la tabla EXAMEN esta repitiendo información que ya existe en la tabla REGISTRO:
...
Entonces se hace uso de 3FN(TERCERA FORMA NORMAL) para eliminar la redundancia de datos y evitar la actualización de datos anómalos.
...
Restricciones:
- El campo REGISTRO_ID_REGISTRO es una clave foránea que referencia a la tabla REGISTRO, por lo que se establece una restricción de clave foránea en este campo. Se establece ON DELETE CASCADE para que al eliminar un registro, se eliminen automáticamente los exámenes relacionados.

- El campo CORRELATIVO_ID_CORRELATIVO es una clave foránea que referencia a la tabla CORRELATIVO, por lo que se establece una restricción de clave foránea en este campo. Se establece ON DELETE CASCADE para que al eliminar un correlativo, se eliminen automáticamente los exámenes relacionados.
...
- **RESPUESTA:** Es un campo numérico que almacena la respuesta del usuario a la pregunta del examen teórico (por ejemplo, 1, 2, 3 o 4, dependiendo de la opción seleccionada).
...
ADD CONSTRAINT UQ_RESP_USUARIO_EXAMEN_PREG
...
- El campo PREGUNTA_PRACTICO_ID_PREGUNTA_PRACTICO es una clave foránea que referencia a la tabla PREGUNTAS_PRACTICO, por lo que se establece una restricción de clave foránea en este campo. Se establece ON DELETE CASCADE para que al eliminar una pregunta del examen práctico, se eliminen automáticamente las respuestas de usuario relacionadas.
